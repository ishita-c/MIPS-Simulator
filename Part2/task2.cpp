...
int count_cycles(inst_data current, int RAD, int CAD)
{
	int ans=0;
	int row=0;
	if(current.aux==true){row=(r[current.field2]+current.field3)/1024;}
	else{row=current.field2/1024;}
	
	if(buffer_row_no==-1){ans+=RAD;}
	else if(buffer_row_no==row){ans=ans;}
	else{ans+=2*RAD;}
	return ans+CAD;
...
bool is_independent_sw(inst_data current) //considering only add, addi, lw and sw
{
	bool ans=false;
	if(current.name=="add" ||current.name=="addi"){ans=true;}
	return ans;
}
bool is_independent_lw(inst_data current, int register_no) //considering only add, addi, lw and sw
{
	bool ans=false;
	if(current.name=="add")
	{
		if(current.aux==true){
			if(current.field1!=register_no && current.field2!=register_no && current.field2!=register_no){ans=true;}
		}else{
			if(current.field1!=register_no && current.field2!=register_no){ans=true;}
		}
	}else if(current.name=="addi"){
		if(current.field1!=register_no && current.field2!=register_no){
			ans=true;
		}
	}
	return ans;
	...
	else if(DRAM[pc].name=="lw")
	{
		...
		if(current_inst.aux){
			...
			if(addr < index || addr >= size || addr%4 != 0){
				...
				int num_of_cycles=count_cycles(current_inst, RAD, CAD);
				pc++;
				for(int i=0; i<num_of_cycles; i++){
					if(is_independent_lw(DRAM[pc], current_inst.field1)==true && pc<index){run_instruction(RAD, CAD);}
					else{break;}
				}
				cycle=current_cycle;
	...
	else if(DRAM[pc].name=="sw")
	{
		if(current_inst.aux){		
			...
			if(addr < index || addr >= size || addr%4 != 0){
				...
				pc++;
				for(int i=0; i<num_of_cycles; i++){
					if(is_independent_sw(DRAM[pc])==true && pc<index){run_instruction(RAD, CAD);}
					else{break;}
				}
				cycle=current_cycle;
